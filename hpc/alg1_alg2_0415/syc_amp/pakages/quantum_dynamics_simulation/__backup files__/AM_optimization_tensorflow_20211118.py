...
class AM_optimize():
    def __init__(self, detuning_list, gate_duration, segments_number, theta, eta):
        ...
        self.entangled_ions_num = 2          # 用于约束离子链中多少个离子上打光，这个值应该比总离子数目小
        self.eta = eta
        self.symmetry = True        #   设计波形是否考虑对称性
        self.segments_num_even = True if self.segments_number % 2 == 0 else False     # 判断segment数量为 奇数偶数
        self.X_init = self.X_initial(symmetry = self.symmetry)   # P的shape 为N个离子，每个离子 N_seg段参数
        self.X_zeros = np.array( [[0]*self.segments_number] * (self.N_ions - self.entangled_ions_num) )
        #   计时结束
        time_end = time.time()
        print('>>>>>>>>>> P and G calculate time used is:',time_end-time_start,'seconds')

    def X_initial(self, symmetry = False):
        if symmetry == False:
            res = np.array( [[1.0]*self.segments_number] * self.entangled_ions_num )
        if symmetry == True:
            if self.segments_num_even == True:
                res = np.array( [[1.0]*int(self.segments_number/2)] * self.entangled_ions_num )
            else:
                res = np.array( [[1.0]*(int(self.segments_number/2)+1)] * self.entangled_ions_num )
        return res

    #   定义优化的cost function，delta_theta 和 alpha的共同贡献
    ...
    def X_append_zeors(self,x):
        ...
        x = self.symmetry_amps_x_extend(x, symmetry=self.symmetry)
        self.append_zeros_res = np.append(x, self.X_zeros)     #   只考虑前几个离子纠缠
        return self.append_zeros_res
    ...
    def cost_function_value(self, x):
        ...
        #   将每个离子上的光强设置成一样  ===========================
        for index in range(1,self.entangled_ions_num):
            ions_amp_x[index] = ions_amp_x[0]
    ...
    def _optimizer_AM(self):
        ...
        # self.optim_results = minimize(self.cost_function_value, self.start, method='SLSQP', bounds=bnds, tol=1e-12)
        #   计时结束
        time_end = time.time()
        print('>>>>>>>>>> optimization time used is:',time_end-time_start,'seconds')
        self.optim_results_position = self.X_append_zeors( self.optim_results.x )
    ...
    def draw_optimize_process(self):
        ...
        plt.title('cost function value v'
                  '..s. iteration steps')
        plt.show()
    ...
    def trajectory_plot(self, steps):  # 画出来相空间轨迹，alpha
        ...
        for i in np.arange(self.mode_number):
            ...
            for j in np.arange(self.N_ions):
                ...
                if  i == 0 and j == 0:      #   添加legends
                    self.axs[i, j].legend(['seg '+str(seg) for seg in range(len(segment_amps))], loc='best')
                temp_alpha_mode_max.append( max(np.sqrt( alpha_sci[0] ** 2 + alpha_sci[1] ** 2 )) )
            self.alpha_modes_simu_max_trunc.append( int( 2*(max(temp_alpha_mode_max))**2)+1 )
        print('alpha max are:', self.alpha_modes_simu_max_trunc)
        plt.show()
...
#   this is the functions back-up,  which are not used in current codes
