...
plt.plot(list(range(2,11)),[2*n*1e2*ms for n in range(2,11)],label = 'N$\Gamma t$')


N = list(heating_error.keys())
N.sort()
print(heating_error)
plt.plot(N,[heating_error[n]/tau_lst[n]*ms for n in N],label = 'numerical')
...
N.sort()
print(my_bound_26)
plt.plot(N,[my_bound_26[n]/tau_lst[n]*ms for n in N],label = 'mybound_eq26')



plt.xlabel('N')
plt.ylabel('infid/t($m s$)')
plt.title('infid-N (t normalized) (middle 1/4 & middle 3/4 ions illuminated)')
